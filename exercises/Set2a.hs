...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = if length xs < n then xs else takeLast n xs
...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = begining ++ [x] ++ ending
    where begining = take i xs
          ending = drop (i+1) xs
...
isPalindrome str = if (head str == last str)
                   then isPalindrome (substring 1 (length str - 1) str)
                   else False
...
-- safeIndex xs i gets the element at index i in the list xs.
-- If i is not a valid index, Nothing is returned.
--
-- Examples:
